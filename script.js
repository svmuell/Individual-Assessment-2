...
function checkUserStatus() {
  ...
  if (currentUser && !document.getElementById('login-form') && !document.getElementById('register-form')) {
...
function loadProducts() {
  ...
  if (products.length === 0) {
    container.innerHTML = '<p class="warning-message">No products available at the moment.</p>';
    return;
  }
  ...
  products.forEach(product => {
    ...
    productCard.innerHTML = `
      <img src="${product.image}" alt="${product.name}" onerror="this.src='https://via.placeholder.com/400x300/0b5fff/ffffff?text=Product+Image'">
...
function loadCart() {
  ...
  if (cart.length === 0) {
    ...
    if (subtotalElement) subtotalElement.textContent = 'JMD 0';
    if (discountElement) discountElement.textContent = '-JMD 0';
    if (taxElement) taxElement.textContent = 'JMD 0';
    if (totalElement) totalElement.textContent = 'JMD 0';
    ...
    row.innerHTML = `
      <td>
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          <img src="${item.image}" alt="${item.name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;" onerror="this.src='https://via.placeholder.com/50x50/0b5fff/ffffff?text=Product'">
          ${item.name}
        </div>
      </td>
      ...
      <td>
        <div class="quantity-controls">
          ...
          <input type="number" class="quantity-input" value="${item.quantity}" min="1" onchange="updateQuantity(${item.id}, parseInt(this.value))" aria-label="Quantity">
      ...
      <td><button class="btn ghost" onclick="removeFromCart(${item.id})" aria-label="Remove item">Remove</button></td>
    `;
...
function showMessage(message, type = 'info') {
  ...
  const existingMessages = document.querySelectorAll('.message, .success-message, .warning-message');
  ...
  if (main) {
    ...
    document.body.insertBefore(messageMessageElement, document.body.firstChild);
  }
...
function generateInvoice() {
  const invoiceNumber = localStorage.getItem('lastInvoice') || 'INV-' + Date.now();
  ...
  // Update billed to information - FIXED: Using proper vanilla JavaScript
  if (checkoutData.shipping && checkoutData.shipping.name) {
    // Find the paragraph that contains "Billed To"
    const paragraphs = document.querySelectorAll('p');
    paragraphs.forEach(p => {
      if (p.textContent.includes('Billed To:')) {
        p.innerHTML = `<strong>Billed To:</strong> ${checkoutData.shipping.name}`;
      }
    });
    
    // Also update payment method if available
    paragraphs.forEach(p => {
      if (p.textContent.includes('Payment Method:') && checkoutData.payment) {
        p.innerHTML = `<strong>Payment Method:</strong> ${checkoutData.payment.method}`;
      }
    });
  }
  ...
  // Clear cart and checkout data after generating invoice
  cart = [];
  localStorage.setItem('cart', JSON.stringify(cart));
